...
// Vendored from react-bits (DavidHDev/react-bits, Animations/LaserFlow).
// WebGL laser-beam + volumetric-fog effect. The GLSL is kept verbatim; the
// wrapper is ported to TypeScript, the `./LaserFlow.css` import is dropped
// (the `.laser-flow-container` rule lives in components/reactbits/reactbits.css),
// and the uniforms are typed. Renders an opaque black canvas whose bright
// beam is composited over a dark surface with `mix-blend-mode: screen` by the
// caller (see LaserFlowLayer).
...
export function LaserFlow({
  ...
  // Seeded in the mount effect (not during render — performance.now() is impure).
  ...
  useEffect(() => {
    ...
    const adjustDprIfNeeded = (now: number) => {
      ...
      const avgFps = samples.reduce((s, b) => s + b, 0) / samples.length;

      let next = currentDprRef.current;
      const base = baseDprRef.current;

      if (avgFps < lowerThresh) {
        next = clampV(currentDprRef.current * 0.85, dprFloor, base);
      } else if (avgFps > upperThresh && currentDprRef.current < base) {
        next = clampV(currentDprRef.current * 1.1, dprFloor, base);
      }
